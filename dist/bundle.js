...
`)}function D(n){return n.replace(/\\/g,"").replace(/\n/g," ").replace(/\r/g,"").replace(/"/g,"'")}class Fe extends HTMLElement{signals=[];selectedName=null;constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render()}setSignals(e){this.signals=e,this.render()}setSelected(e){this.selectedName=e,this.render()}render(){if(!this.shadowRoot)return;const e=this.signals.map(t=>{const s=this.selectedName===t.name,a=t.byte_order==="little_endian"?"LE":"BE",i=t.is_unsigned?"U":"S",r=t.unit||"-",c=t.is_multiplexer?"M":t.multiplexer_value!==null?`m${t.multiplexer_value}`:"-";return`
    ...
    `}updateBitBar(){if(!this.shadowRoot||!this.editingSignal)return;const e=this.message.dlc*8;if(e===0)return;const t=this.editingSignal.start_bit,s=this.editingSignal.length,a=this.shadowRoot.querySelector(".cv-bit-layout-header span:last-child");a&&(a.textContent=`${this.editingSignal.name||""}: ${t} - ${t+s-1}`);const i=this.shadowRoot.querySelector(".cv-bit-bar");if(i){const r=this.getActiveSignals(),c=this.editingSignal.byte_order,h=(l,d,v)=>_(l,d,v),p=(l,d,v,g)=>{const m=h(l,d,v);for(const b of r){if(b.name===g)continue;const y=b.name===this.editingSignal.name,C=y?t:b.start_bit,$=y?s:b.length,M=y?c:b.byte_order,I=h(C,$,M);if(m.start<=I.end&&I.start<=m.end)return!0}return!1},o=r.map((l,d)=>{const v=l.name===this.editingSignal.name,g=v?t:l.start_bit,m=v?s:l.length,b=v?c:l.byte_order,y=h(g,m,b),C=y.start/e*100,$=m/e*100,M=p(g,m,b,l.name),I=M?"#ef4444":v?"#3b82f6":U(d),W=v?1:.5,Z=b==="big_endian"?"BE":"LE";return`<div class="${["cv-bit-segment",v?"current":"",M?"overlap":""].filter(Boolean).join(" ")}"
                     style="left: ${C}%; width: ${$}%; background: ${I}; opacity: ${W};"
                     title="${l.name} (${Z}): bits ${y.start}-${y.end}${M?" (OVERLAP!)":""}">
                  ${$>8?l.name:""}
                </div>`}).join("");if(this.isAddingSignal&&this.editingSignal){const l=h(t,s,c),d=l.start/e*100,v=s/e*100,g=p(t,s,c,""),m=g?"#ef4444":"#3b82f6",b=c==="big_endian"?"BE":"LE",C=`<div class="${["cv-bit-segment","current",g?"overlap":""].filter(Boolean).join(" ")}"
                              style="left: ${d}%; width: ${v}%; background: ${m}; opacity: 1;"
                              title="New (${b}): bits ${l.start}-${l.end}${g?" (OVERLAP!)":""}">
                           ${v>8?this.editingSignal.name||"New":""}
