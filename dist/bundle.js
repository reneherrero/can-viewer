...
`)}class _e extends HTMLElement{signals=[];selectedName=null;constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render()}setSignals(e){this.signals=e,this.render()}setSelected(e){this.selectedName=e,this.render()}render(){if(!this.shadowRoot)return;const e=this.signals.map(t=>{const s=this.selectedName===t.name,a=t.byte_order==="little_endian"?"LE":"BE",n=t.is_unsigned?"U":"S",r=t.unit||"-",c=t.is_multiplexer?"M":t.multiplexer_value!==null?`m${t.multiplexer_value}`:"-";return`
    ...
    `}updateBitBar(){if(!this.shadowRoot||!this.editingSignal)return;const e=this.message.dlc*8;if(e===0)return;const t=this.editingSignal.start_bit,s=this.editingSignal.length,a=this.shadowRoot.querySelector(".cv-bit-layout-header span:last-child");a&&(a.textContent=`${this.editingSignal.name||""}: ${t} - ${t+s-1}`);const n=this.shadowRoot.querySelector(".cv-bit-bar");if(n){const r=this.getActiveSignals(),c=this.editingSignal.byte_order,v=(o,h,u)=>L(o,h,u),d=(o,h,u,p)=>{const m=v(o,h,u);for(const f of r){if(f.name===p)continue;const w=f.name===this.editingSignal.name,C=w?t:f.start_bit,k=w?s:f.length,M=w?c:f.byte_order,I=v(C,k,M);if(m.start<=I.end&&I.start<=m.end)return!0}return!1},l=r.map((o,h)=>{const u=o.name===this.editingSignal.name,p=u?t:o.start_bit,m=u?s:o.length,f=u?c:o.byte_order,w=v(p,m,f),C=w.start/e*100,k=m/e*100,M=d(p,m,f,o.name),I=M?"#ef4444":u?"#3b82f6":U(h),J=u?1:.5,K=f==="big_endian"?"BE":"LE";return`<div class="${["cv-bit-segment",u?"current":"",M?"overlap":""].filter(Boolean).join(" ")}"
                     style="left: ${C}%; width: ${k}%; background: ${I}; opacity: ${J};"
                     title="${o.name} (${K}): bits ${w.start}-${w.end}${M?" (OVERLAP!)":""}">
                  ${k>8?o.name:""}
                </div>`}).join("");if(this.isAddingSignal&&this.editingSignal){const o=v(t,s,c),h=o.start/e*100,u=s/e*100,p=d(t,s,c,""),m=p?"#ef4444":"#3b82f6",f=c==="big_endian"?"BE":"LE",C=`<div class="${["cv-bit-segment","current",p?"overlap":""].filter(Boolean).join(" ")}"
