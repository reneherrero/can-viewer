(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const r of i)if(r.type==="childList")for(const n of r.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&a(n)}).observe(document,{childList:!0,subtree:!0});function t(i){const r={};return i.integrity&&(r.integrity=i.integrity),i.referrerPolicy&&(r.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?r.credentials="include":i.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(i){if(i.ep)return;i.ep=!0;const r=t(i);fetch(i.href,r)}})();class b{invoke;listen;openDialog;saveDialog;constructor(){this.invoke=async()=>{throw new Error("Tauri not initialized")},this.listen=async()=>()=>{},this.openDialog=async()=>null,this.saveDialog=async()=>null}async init(){const e=window.__TAURI__;if(!e)throw new Error("Tauri API not available");this.invoke=e.core.invoke,this.listen=e.event.listen,this.openDialog=e.dialog.open,this.saveDialog=e.dialog.save}async loadDbc(e){return await this.invoke("load_dbc",{path:e})}async clearDbc(){await this.invoke("clear_dbc")}async getDbcInfo(){return await this.invoke("get_dbc_info")}async getDbcPath(){return await this.invoke("get_dbc_path")}async decodeFrames(e){return await this.invoke("decode_frames",{frames:e})}async loadMdf4(e){return await this.invoke("load_mdf4",{path:e})}async exportLogs(e,t){return await this.invoke("export_logs",{path:e,frames:t})}async listCanInterfaces(){return await this.invoke("list_can_interfaces")}async startCapture(e){await this.invoke("start_capture",{interface:e})}async stopCapture(){await this.invoke("stop_capture")}async getInitialFiles(){return await this.invoke("get_initial_files")}async openFileDialog(e){return await this.openDialog({multiple:!1,filters:e})}async saveFileDialog(e,t){return await this.saveDialog({filters:e,defaultPath:t})}onCanFrame(e){let t=null;return this.listen("can-frame",a=>{e(a.payload)}).then(a=>{t=a}),()=>{t&&t()}}onDecodedSignal(e){let t=null;return this.listen("decoded-signal",a=>{e(a.payload)}).then(a=>{t=a}),()=>{t&&t()}}onCaptureError(e){let t=null;return this.listen("capture-error",a=>{e(a.payload)}).then(a=>{t=a}),()=>{t&&t()}}}function g(s){return`
  ...
  `}function x(s){return`
    <div class="cv-tabs">
      ${s.showMdf4Tab?'<button class="cv-tab-btn" data-tab="mdf4">MDF4</button>':""}
      ${s.showLiveTab?'<button class="cv-tab-btn" data-tab="live">Live Capture</button>':""}
      ${s.showDbcTab?'<button class="cv-tab-btn" data-tab="dbc">DBC</button>':""}
    </div>
  `}function y(){return`
  ...
  `).join(""):'<div class="cv-dbc-no-file">No DBC file loaded</div>'}function Z(s){return s.signals.length===0?'<div class="cv-dbc-empty">No signals defined for this message</div>':s.signals.map(e=>`
  ...
  `).join("")}function ee(s){return`ID: 0x${s.id.toString(16).toUpperCase()} | DLC: ${s.dlc}${s.sender?" | TX: "+s.sender:""}`}function te(s){return'<option value="">Select CAN interface...</option>'+s.map(e=>`<option value="${e}">${e}</option>`).join("")}class se extends HTMLElement{dbcInfo=null;selectedMessageId=null;constructor(){super()}setDbcInfo(e){this.dbcInfo=e,this.selectedMessageId=null,this.render()}render(){this.renderMessagesList(),this.renderDetails()}renderMessagesList(){const e=this.querySelector(".cv-dbc-messages-scroll");e&&(e.innerHTML=Q(this.dbcInfo,this.selectedMessageId),this.bindMessageEvents(e))}bindMessageEvents(e){e.querySelectorAll(".cv-dbc-message-item").forEach(t=>{t.addEventListener("click",()=>{const a=parseInt(t.dataset.id||"0");this.selectMessage(a)})})}selectMessage(e){this.selectedMessageId=e,this.render();const t=this.getSelectedMessage();t&&this.dispatchEvent(new CustomEvent("message-selected",{detail:{message:t},bubbles:!0}))}getSelectedMessage(){return this.selectedMessageId===null||!this.dbcInfo?.messages?null:this.dbcInfo.messages.find(e=>e.id===this.selectedMessageId)||null}renderDetails(){const e=this.querySelector(".cv-dbc-details-title"),t=this.querySelector(".cv-dbc-details-subtitle"),a=this.querySelector(".cv-dbc-details-scroll"),i=this.getSelectedMessage();if(!i){e&&(e.textContent="Select a message"),t&&(t.textContent=""),a&&(a.innerHTML='<div class="cv-dbc-empty">Select a message to view its signals</div>');return}e&&(e.textContent=i.name),t&&(t.textContent=ee(i)),a&&(a.innerHTML=Z(i))}}customElements.define("cv-dbc-viewer",se);class ae extends HTMLElement{isCapturing=!1;hasFrames=!1;constructor(){super()}connectedCallback(){this.bindEvents()}bindEvents(){const e=this.querySelector("#refreshInterfacesBtn"),t=this.querySelector("#startCaptureBtn"),a=this.querySelector("#stopCaptureBtn"),i=this.querySelector("#exportLogsBtn"),r=this.querySelector("#interfaceSelect");e?.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("refresh-interfaces",{bubbles:!0}))}),t?.addEventListener("click",()=>{const n=this.getSelectedInterface();n&&this.dispatchEvent(new CustomEvent("start-capture",{detail:{interface:n},bubbles:!0}))}),a?.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("stop-capture",{bubbles:!0}))}),i?.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("export-logs",{bubbles:!0}))}),r?.addEventListener("change",()=>this.updateButtons()),r?.addEventListener("mousedown",()=>{this.dispatchEvent(new CustomEvent("refresh-interfaces",{bubbles:!0}))})}setInterfaces(e){const t=this.querySelector("#interfaceSelect");t&&(t.innerHTML=te(e)),this.updateButtons()}getSelectedInterface(){return this.querySelector("#interfaceSelect")?.value||""}setCaptureStatus(e){this.isCapturing=e;const t=this.querySelector("#statusDot"),a=this.querySelector("#statusText"),i=this.querySelector("#stopCaptureBtn");t?.classList.toggle("connected",e),a&&(a.textContent=e?"Capturing...":"Idle"),i&&(i.disabled=!e),this.updateButtons()}setHasFrames(e){this.hasFrames=e,this.updateButtons()}updateButtons(){const e=this.querySelector("#interfaceSelect"),t=this.querySelector("#startCaptureBtn"),a=this.querySelector("#exportLogsBtn");t&&e&&(t.disabled=!e.value||this.isCapturing),a&&(a.disabled=!this.hasFrames||this.isCapturing)}}customElements.define("cv-capture-controls",ae);class ie extends HTMLElement{api=null;state;shadow;elements={};unlisteners=[];framesTable=null;signalsPanel=null;filtersPanel=null;dbcViewer=null;captureControls=null;frameRenderPending=!1;signalRenderPending=!1;lastStatsUpdate=0;frameBuffer=[];flushTimer=null;constructor(){super(),this.state=u(),this.shadow=this.attachShadow({mode:"open"})}setApi(e){this.api=e,this.setupEventListeners()}setConfig(e){this.state=u(e),this.render()}connectedCallback(){this.render()}disconnectedCallback(){this.unlisteners.forEach(e=>e()),this.unlisteners=[]}setupEventListeners(){this.api&&(this.unlisteners.push(this.api.onCanFrame(e=>{this.frameBuffer.push(e),this.scheduleFrameFlush()}),this.api.onDecodedSignal(e=>{this.state.isCapturing||(q(this.state,e),this.scheduleSignalRender())}),this.api.onCaptureError(e=>{this.showMessage(e,"error"),this.updateCaptureUI(!1)})),this.loadInitialFiles())}scheduleFrameFlush(){this.flushTimer===null&&(this.flushTimer=window.setTimeout(()=>{this.flushTimer=null,this.flushFrameBuffer()},100))}flushFrameBuffer(){if(this.frameBuffer.length!==0){for(const e of this.frameBuffer)$(this.state,e);this.frameBuffer=[],this.scheduleFrameRender()}}scheduleFrameRender(){this.frameRenderPending||(this.frameRenderPending=!0,this.state.isCapturing?requestAnimationFrame(()=>{this.frameRenderPending=!1,this.renderFramesThrottled()}):requestAnimationFrame(()=>{this.frameRenderPending=!1,this.renderFrames()}))}scheduleSignalRender(){this.signalRenderPending||(this.signalRenderPending=!0,requestAnimationFrame(()=>{this.signalRenderPending=!1,this.renderSignals()}))}renderFramesThrottled(){o(this.state),this.framesTable?.setFrames(this.state.filteredFrames),this.updateFrameCount(),this.captureControls?.setHasFrames(this.state.frames.length>0);const e=performance.now();e-this.lastStatsUpdate>=500&&(this.lastStatsUpdate=e,this.updateFrameStats())}async loadInitialFiles(){if(this.api)try{const e=await this.api.getInitialFiles();e.dbc_path&&await this.loadDbc(e.dbc_path),e.mdf4_path&&await this.loadMdf4(e.mdf4_path)}catch(e){console.error("Failed to load initial files:",e)}}render(){this.shadow.innerHTML=`<style>${V}</style>${g(this.state.config)}`,this.cacheElements(),this.bindEvents(),this.switchTab(this.state.activeTab)}cacheElements(){T.forEach(e=>{const t=this.shadow.getElementById(e);t&&(this.elements[e]=t)}),this.framesTable=this.shadow.querySelector("cv-frames-table"),this.signalsPanel=this.shadow.querySelector("cv-signals-panel"),this.filtersPanel=this.shadow.querySelector("cv-filters-panel"),this.dbcViewer=this.shadow.querySelector("cv-dbc-viewer"),this.captureControls=this.shadow.querySelector("cv-capture-controls"),this.framesTable?.setMessageNameLookup(e=>h(this.state,e))}renderFrames(){o(this.state),this.framesTable?.setFrames(this.state.filteredFrames),this.updateFrameCount(),this.updateFrameStats(),this.captureControls?.setHasFrames(this.state.frames.length>0)}renderSignals(){this.signalsPanel?.setSignals(this.state.signals)}clearSignalsPanel(){this.signalsPanel?.showEmpty()}renderDbcMessages(){this.dbcViewer?.setDbcInfo(this.state.dbcInfo)}updateFrameCount(){this.filtersPanel?.setFilterCount(this.state.filteredFrames.length,this.state.frames.length)}updateFrameStats(){const e=R(this.state),t=this.elements.statMsgCount,a=this.elements.statFrameRate,i=this.elements.statDeltaTime,r=this.elements.statBusLoad;t&&(t.textContent=String(e.uniqueMessages)),a&&(a.textContent=`${Math.round(e.frameRate)}/s`),i&&(i.textContent=e.avgDeltaMs>0?`${e.avgDeltaMs.toFixed(1)}ms`:"-"),r&&(r.textContent=`${e.busLoad.toFixed(1)}%`)}bindEvents(){this.shadow.querySelectorAll(".cv-tab-btn").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.tab;t&&this.switchTab(t)})}),this.elements.dbcStatusBtn?.addEventListener("click",()=>this.switchTab("dbc")),this.elements.loadDbcBtnTab?.addEventListener("click",()=>this.promptLoadDbc()),this.elements.clearDbcBtn?.addEventListener("click",()=>this.clearDbc()),this.elements.loadMdf4Btn?.addEventListener("click",()=>this.promptLoadMdf4()),this.elements.clearDataBtn?.addEventListener("click",()=>this.clearAllData()),this.elements.clearLiveDataBtn?.addEventListener("click",()=>this.clearAllData()),this.framesTable?.addEventListener("frame-selected",e=>{const t=e.detail.frame,a=e.detail.index;z(this.state,a),this.decodeFrame(t)}),this.filtersPanel?.addEventListener("filter-change",e=>{const t=e.detail;this.state.filters=t,o(this.state),this.renderFrames(),this.clearSignalsPanel()}),this.captureControls?.addEventListener("refresh-interfaces",()=>this.loadInterfaces()),this.captureControls?.addEventListener("start-capture",e=>{const t=e.detail.interface;this.startCaptureOnInterface(t)}),this.captureControls?.addEventListener("stop-capture",()=>this.stopCapture()),this.captureControls?.addEventListener("export-logs",()=>this.exportLogs())}switchTab(e){N(this.state,e),this.shadow.querySelectorAll(".cv-tab-btn").forEach(a=>{a.classList.toggle("active",a.dataset.tab===e)}),this.shadow.querySelectorAll(".cv-tab-content").forEach(a=>{a.classList.toggle("active",a.id===`${e}Tab`)});const t=e==="dbc";this.elements.tablesContainer?.classList.toggle("hidden",t),this.elements.filtersSection?.classList.toggle("hidden",t),this.elements.dbcViewer?.classList.toggle("hidden",!t),t&&this.loadDbcInfo()}async promptLoadDbc(){if(this.api)try{const e=await this.api.openFileDialog([{name:"DBC Files",extensions:["dbc"]}]);e&&await this.loadDbc(e)}catch(e){this.showMessage(String(e),"error")}}async loadDbc(e){if(this.api)try{const t=await this.api.loadDbc(e),a=e.split("/").pop()?.split("\\").pop()||e;await this.loadDbcInfo(),this.updateDbcStatusUI(!0,a),this.showMessage(t),this.renderFrames();const i=H(this.state);i&&await this.decodeFrame(i)}catch(t){this.showMessage(String(t),"error")}}async clearDbc(){if(this.api)try{await this.api.clearDbc(),p(this.state,!1),this.updateDbcStatusUI(!1),this.state.activeTab==="dbc"&&this.renderDbcMessages(),this.showMessage("DBC cleared")}catch(e){this.showMessage(String(e),"error")}}async loadDbcInfo(){if(this.api)try{const e=await this.api.getDbcInfo();p(this.state,!!e,e),this.renderDbcMessages()}catch(e){console.error("Failed to load DBC info:",e)}}updateDbcStatusUI(e,t=""){const a=this.elements.dbcStatusBtn,i=this.elements.clearDbcBtn;a&&(a.textContent=e?`DBC: ${t}`:"No DBC loaded",a.classList.toggle("loaded",e)),i&&(i.disabled=!e),this.elements.signalsPanel?.classList.toggle("hidden",!e),this.elements.tablesContainer?.classList.toggle("with-signals",e)}async promptLoadMdf4(){if(this.api)try{const e=await this.api.openFileDialog([{name:"MDF4 Files",extensions:["mf4","mdf","mdf4","MF4","MDF","MDF4"]}]);e&&await this.loadMdf4(e)}catch(e){this.showMessage(String(e),"error")}}async loadMdf4(e){if(!this.api)return;const t=this.elements.loadMdf4Btn;try{t&&(t.disabled=!0,t.textContent="Loading...");const[a]=await this.api.loadMdf4(e);this.state.frames=a,this.state.signals=[],this.state.selectedFrameIndex=null,this.renderFrames(),this.clearSignalsPanel(),this.showMessage(`Loaded ${a.length} frames`)}catch(a){this.showMessage(String(a),"error")}finally{t&&(t.disabled=!1,t.textContent="Load MDF4 File")}}clearAllData(){P(this.state),this.renderFrames(),this.clearSignalsPanel()}async loadInterfaces(){if(this.api)try{const e=await this.api.listCanInterfaces();this.captureControls?.setInterfaces(e)}catch(e){console.log("Could not load interfaces:",e)}}async startCaptureOnInterface(e){if(this.api)try{this.clearAllData(),this.updateCaptureUI(!0),await this.api.startCapture(e),this.showMessage(`Capturing on ${e}`)}catch(t){this.updateCaptureUI(!1),this.showMessage(String(t),"error")}}async stopCapture(){if(this.api)try{await this.api.stopCapture(),this.flushTimer!==null&&(clearTimeout(this.flushTimer),this.flushTimer=null),this.flushFrameBuffer(),this.updateCaptureUI(!1),this.showMessage("Capture stopped")}catch(e){this.showMessage(String(e),"error")}}updateCaptureUI(e){A(this.state,e),this.captureControls?.setCaptureStatus(e)}async exportLogs(){if(!(!this.api||this.state.frames.length===0))try{const e=await this.api.saveFileDialog([{name:"MDF4 Files",extensions:["mf4"]}],"capture.mf4");if(!e)return;const t=await this.api.exportLogs(e,this.state.frames);this.showMessage(`Exported ${t} frames to MDF4`)}catch(e){this.showMessage(String(e),"error")}}async decodeFrame(e){if(!this.api||!this.state.dbcLoaded){this.clearSignalsPanel();return}try{this.state.signals=await this.api.decodeFrames([e]),this.renderSignals()}catch(t){console.error("Failed to decode frame:",t),this.clearSignalsPanel()}}showMessage(e,t="success"){const a=document.createElement("div");a.className=`cv-message ${t}`,a.textContent=e,this.shadow.appendChild(a),setTimeout(()=>a.remove(),3e3)}}customElements.define("can-viewer",ie);async function v(){const s=new b;await s.init();const e=document.querySelector("can-viewer");e&&e.setApi(s)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",v):v();
